...
class DTLS(nn.Module):
    ...
    def transform_func_noise(self, img, target_size, noise=None, random_mean=None, noise_level=256):
        ...
        if n <= noise_level:
            ...
            # noise = torch.randn(img_1.shape[0], 3, 2, 2).to(img_1.get_device())
...
class Trainer(object):
    ...
    def evaluation(self, num_sample=1, batch_size=16):
        ...
        for i in range(num_sample):
            random_vector = self.random_vector(16)
            blur_img_set, sample_hr = self.ema_model.sample(batch_size=batch_size, img=random_vector, save_folder=self.results_folder)

            # utils.save_image((blur_img_set + 1) /2, str(self.results_folder /  f'random_vector_{i}.png'), nrow=4)
            # utils.save_image((sample_hr + 1) /2, str(self.results_folder /  f'result_{i}.png'), nrow=4)
